...
export default class ContactForm extends Component {
  ...
  render() {
    ...
    const slideButton = this.state.slide
      ? `${style.button_slide}`
      : `${style.button}`
    // on successful submitting, thanks message animation
    const slideText = this.state.slide
      ? `${style.thanks_active}`
      : `${style.thanks}`
    ...
    return (
      ...
      >
        ...
        <button
          ...
          style={this.state.error ? { border: 'solid 1px red' } : null}
        >
          {this.state.error
        ...
        <span className={slideText}>
          {timeOfDay} Thanks {this.state.name} for your message{timeOff}
        </span>
      </form>
    )
  }
